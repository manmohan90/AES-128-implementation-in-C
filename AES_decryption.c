...
byte y[16]; //  array for 128 bit ciphertext
byte y_mat[4][4]; // 4x4 matrix for 128 bit ciphertext
...
//applying invshift rows on y_mat
void invshiftRows()
{
    byte y_shift[4][4]; // 4x4 matrix store values after shift rows operation
    for(int i=0;i<4;i++)
    {
        for(int j=0;j<4;j++)
        {
            int k =(j-i+4)%4;
            y_shift[i][j] = y_mat[i][k]; 
        }
    }
    for(int i=0;i<4;i++) 
   {
       for(int j=0;j<4;j++)
        y_mat[i][j]=y_shift[i][j];
   } 
...
int multiply(int x,int y)
{
    return  (  ((y & 1) * x) ^                              \
      ((y>>1 & 1) *  mulfun(x)) ^                       \
      ((y>>2 & 1) * mulfun(mulfun(x))) ^                \
      ((y>>3 & 1) * mulfun(mulfun(mulfun(x)))) ^         \
      ((y>>4 & 1) * mulfun(mulfun(mulfun(mulfun(x)))))) ;
}
//applying mix columns on y_mat 
void invmixColumn()
{
   
    int arr[4];  // temp array 
    byte y_mixi[4][4];  //4x4 matrix store values after mix column operation
    for(int i=0; i<4; ++i)  
    {  
        for(int j=0; j<4; ++j)  
            arr[j] = y_mat[j][i]; // taking column values in arr
        y_mixi[0][i] =  multiply(arr[0],0x0e)^multiply(arr[1],0x0b)^multiply(arr[2],0x0d)^multiply(arr[3],0x09);   
        y_mixi[1][i] =  multiply(arr[0],0x09)^multiply(arr[1],0x0e)^multiply(arr[2],0x0b)^multiply(arr[3],0x0d); 
        y_mixi[2][i] =  multiply(arr[0],0x0d)^multiply(arr[1],0x09)^multiply(arr[2],0x0e)^multiply(arr[3],0x0b);  
        y_mixi[3][i] =  multiply(arr[0],0x0b)^multiply(arr[1],0x0d)^multiply(arr[2],0x09)^multiply(arr[3],0x0e);
   ...
   {
       for(int j=0;j<4;j++)
        y_mat[i][j]=y_mixi[i][j];
   } 
   
 }
//applying invs-box for invsubbyte operation
byte invs_box(byte value)
{
   int row = value>>4;   // getting row number
   int col = value&(0xf); // getting column number
   return Inv_S_Box[row][col];
}   
// applying invsubbyte operation
void invsubbytes()
{
   for(int i=0;i<4;i++)
   {
       for(int j=0;j<4;j++)
        y_mat[i][j] = invs_box(y_mat[i][j]);
   }
    
}
//Add round key operation - AES decryption
void AddRoundKey_dec(int a)
{
    for(int i=0;i<4;i++)
    {
       for(int j=0;j<4;j++)
       {
         y_mat[i][j] = (y_mat[i][j])^(round_keys[10-a][i][j]);
       }
    }
...
int main()
{
    printf("Input 128 bits ciphertext:\n");
    for(int i=0;i<16;i++)
        scanf("%x",&y[i]); 
    int k=0;
    for(int j=0;j<4;j++)
    ...
    printf("128-bit decrypted plaintext generated after 10 round function is :\n");
    for(int j=0;j<4;j++)
    {
        for(int i=0;i<4;i++)
         printf("%0.2x ",y_mat[i][j]);
       // printf(" ");
    }
    printf("\n");
